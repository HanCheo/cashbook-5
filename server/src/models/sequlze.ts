...
let sequlz: Sequelize | undefined;

export async function connectDB() {
  if (typeof sequlz === 'undefined') {
    if (process.env.SEQUELIZE_CONNECT === undefined) {
      throw Error('SEQUELIZE_CONNECT is not found');
    }

    const yamltext = await fs.readFile(process.env.SEQUELIZE_CONNECT, 'utf8');
    const params: any = load(yamltext, { json: true });
